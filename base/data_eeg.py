...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        self.data_paths = [
            os.path.join(self.data_dir, subject, f'{mode}.pt')
        ...
        feature_cache_name = config['data'].get('feature_cache_name', self.name)
        features_filename = os.path.join(
            data_dir,
            f"{feature_cache_name}_{mode}.pt"
        )
        ...
        # =========================================================
        # Build blur transform
        # =========================================================
        if self.config['data']['uncertainty_aware']:
            self.blur_transform = {}

            base_kernel_size = int(config['data']['blur_type']['params']['blur_kernel_size'])
        ...
        # =========================================================
        # Helper: average old/new image feature entries
        # 支持：
        #   tensor[D]
        #   tensor[2,D]
        #   {"feat": tensor[2,D], "stat": tensor[6]}
        # =========================================================
        ...
        def _average_img_feature_entries(entries):
            ...
            if isinstance(first, dict):
                ...
                feat_avg = torch.stack(feats, dim=0).mean(dim=0)
                stat_avg = torch.stack(stats, dim=0).mean(dim=0)
                ctx_avg = torch.stack(ctx_feats, dim=0).mean(dim=0)
        ...
        # =========================================================
        # Load or build image/text feature cache
        # =========================================================
        if os.path.exists(features_filename):
            ...
            if self.config['data']['uncertainty_aware']:
                ...
                # 兼容 tensor 和 dict 两种 feature entry
    ...
    def ImageEncoder(self, images, blur_transform=None):
        ...
        ACVC + context teacher 模式:
            ...
            feat[0]: AWMB fused feature
            feat[1]: original image feature
            stat: AWMB attention statistics
            ctx_feat[0]: low blur feature
            ctx_feat[1]: mid blur feature
            ctx_feat[2]: high blur feature
        """
        ...
        image_batch_size = int(self.config['data'].get('feature_extract_batch_size', 32))
        encode_batch_size = int(self.config['data'].get('feature_encode_batch_size', 64))
    ...
    def __getitem__(self, index):
        ...
        text_features = self.text_features[
            self.loaded_data[subject]['text'][trial_index]
        ]
